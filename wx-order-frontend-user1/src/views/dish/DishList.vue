...
const imageGroups = computed(() => {
  ...
  for (const img of imageList.value) {
    ...
    const shortName =
      slashIdx > 0 ? img.name.substring(slashIdx + 1) : img.name;
...
async function fetchImages() {
  try {
    ...
    const res = await axios.get(
      "http://localhost:8080/api/admin/upload/images",
      {
...
async function autoFillNutrition() {
  ...
  try {
    ...
    const res = await axios.post(
      "http://localhost:8080/api/admin/dishes/nutrition-lookup",
